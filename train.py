...
if __name__ == "__main__":
    ...
    # For 학습
    model_name = 'efficientnet_b3a_aug_ch_trfm_normalize' # 이 이름으로 저장할 폴더들이 생성되고 모델과 로스변화 csv 결과들이 저장이 됩니다!
    ealry_stopping = 5
    k_fold_num = 5 # cross validation을 안하시려면 -1로 설정해주세요
    epoches = 100
    lr = 1e-4
    batch_size = 32
...
        RandomBrightnessContrast(brightness_limit=(-0.3, 0.3), contrast_limit=(-0.3, 0.3), p=0.3),
        Normalize(mean=(0.548, 0.504, 0.479), std=(0.237, 0.247, 0.246), max_pixel_value=255.0, p=1.0),
    ...
    if k_fold_num == -1:
        ...
        train_dataset = TrainDataset(train_img, train_y, transform_train)
        
        # dataset.py에서 구현한 dataset class로 평가 데이터 정의
        valid_dataset = TrainDataset(valid_img, valid_y, transform_valid)
        ...
        print("Train Start!!")
        Train(train_loader, valid_loader, class_weigth, -1)
    ...
    # Cross validation 할때
        ...
        for fold_index, (train_idx, valid_idx) in enumerate(kf.split(img_list, y_list), 1):
            ...
            # dataset.py에서 구현한 dataset class로 훈련 데이터 정의
            train_dataset = TrainDataset(train_list, train_label, transform_train)
            
            # dataset.py에서 구현한 dataset class로 평가 데이터 정의
            valid_dataset = TrainDataset(valid_list, valid_label, transform_valid)
            ...
            print("Train Start!!")
            train(train_loader, valid_loader, class_weigth, fold_index)
