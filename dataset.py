...
# class TrainDataset(Dataset):
    # def __init__(self, csv_paths, img_paths, transform, load_augmentation):
        # self.csv_paths = csv_paths
        # self.img_paths = img_paths
        # self.transform = transform
        # self.load_augmentation = load_augmentation

        # self.X_pathes = self.PathMaker() 
        # self.y = [self.GetLabel(path) for path in self.X_pathes]
        
        # #self.features = pd.read_csv(csv_paths)

    # def __len__(self):
        # return len(self.X_pathes)

    # def __getitem__(self, index):
        # image = Image.open(self.X_pathes[index])
        # #y = self.GetLabel(self.X_pathes[index])
    ...
    # def GetLabel(self, path):
        ...
        # number, sex, race, age = items[0].split('_')
        # mask_status = items[1].split('.')[0]
        # #print(number, sex, race, age, mask_status)
    ...
    # def PathMaker(self):
        # path_list = []
        # img_dir = pd.read_csv(self.csv_paths)#"../input/data/train/train.csv")
        # img_dir_pathes = img_dir['path']
        # img_dir_patheses = [os.path.join(self.img_paths, img_d) for img_d in img_dir_pathes]
        # for f_name in img_dir_patheses:
            # for img_name in os.listdir(f_name):
                # if img_name[0] == '.':
                    # continue
                # if not self.load_augmentation and 'aug' in img_name :
                    # continue
                # #elif ('aug' in img_name and ('incorrect' in img_name or 'normal' in img_name)) or ('aug' not in img_name and 'mask' in img_name):
                # path_list.append(os.path.join(f_name, img_name))
        # return path_list       